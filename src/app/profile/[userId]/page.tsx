...
import {
  Globe, Camera, Coins, MapPin, Settings, Calendar, Sparkles, Handshake, Star,
} from "lucide-react";
...
export default async function ProfilePage({
  ...
  const user = await prisma.user.findUnique({
    where: { id: userId },
    select: {
      id: true, name: true, image: true, bio: true, age: true,
      location: true, bizCoins: true, createdAt: true,
  ...
  const currentUserSkills = !isOwnProfile && session
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-12">
          ...
          <div className="relative flex flex-col items-center px-8 pb-8 pt-12 text-center sm:flex-row sm:items-start sm:gap-8 sm:pb-10 sm:pt-10 sm:text-left">
            ...
            <div className="mt-6 flex-1 sm:mt-2">
              <div className="flex flex-col items-center gap-3 sm:flex-row sm:flex-wrap">
                ...
                {user.bizCoins !== undefined && (
                  <Badge className="border-amber-500/20 bg-amber-500/10 px-3 py-1 text-amber-400">
                    <Coins className="mr-1 h-4 w-4" />{user.bizCoins} BC
                  </Badge>
                )}
                {avgRating && (
                  <Badge className="border-yellow-500/20 bg-yellow-500/10 text-yellow-400">
                    <Star className="mr-1 h-3 w-3 fill-yellow-400" />{avgRating.toFixed(1)}
                  </Badge>
                )}
              </div>
              ...
              {user.businessProfile && (
                <>
                  <p className="mt-2 text-xl font-medium text-gray-300">{user.businessProfile.businessName}</p>
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-x-4 gap-y-2 text-sm text-gray-400 sm:justify-start">
                ...
                <span className="flex items-center gap-1">
                  <Calendar className="h-4 w-4" />{new Date(user.createdAt).toLocaleDateString("en-US", { month: "short", year: "numeric" })}
                </span>
                <span className="flex items-center gap-1">
                  <Sparkles className="h-4 w-4 text-indigo-400" />{offered.length + needed.length} skills
                </span>
              </div>

              {user.bio && <p className="mt-4 max-w-xl text-sm leading-relaxed text-gray-400">{user.bio}</p>}

              <div className="mt-5 flex flex-wrap justify-center gap-3 sm:justify-start">
                {user.businessProfile?.website && (
                  <a href={user.businessProfile.website} target="_blank" rel="noopener noreferrer"
                ...
                {user.businessProfile?.instagramHandle && (
                  <a href={`https://instagram.com/${user.businessProfile.instagramHandle.replace("@", "")}`}
                     target="_blank" rel="noopener noreferrer"
                     className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/[0.03] px-3.5 py-2 text-sm text-gray-300 transition-all hover:border-white/20 hover:bg-white/[0.06] hover:text-white">
                    <Camera className="h-4 w-4" />{user.businessProfile.instagramHandle}
                  </a>
                )}
              </div>
            </div>
          ...
          <Tabs defaultValue="offered" className="w-full">
            <div className="flex items-center justify-between">
              <TabsList className="border-white/10 bg-white/[0.03]">
                <TabsTrigger value="offered" className="data-[state=active]:bg-indigo-500/10 data-[state=active]:text-indigo-400">
                  Skills I Offer ({offered.length})
                </TabsTrigger>
                <TabsTrigger value="needed" className="data-[state=active]:bg-purple-500/10 data-[state=active]:text-purple-400">
              ...
              {isOwnProfile && (
                <Link href="/dashboard/settings">
                  <Button variant="ghost" size="sm" className="text-gray-400 hover:text-white">
                    <Sparkles className="mr-1.5 h-4 w-4" />Manage Skills
                  </Button>
                </Link>
              )}
            </div>
              ...
              {offered.length === 0 ? (
                <Card className="border-white/10 bg-white/[0.03] py-16 text-center">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {isOwnProfile ? "Add skills you can help others with." : "This user hasn't listed any skills yet."}
                  </p>
                </Card>
              ) : (
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {offered.map((skill) => (
                    <Card key={skill.id} className="group border-white/10 bg-white/[0.03] transition-all hover:border-indigo-500/30 hover:bg-white/[0.06]">
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Badge variant="outline" className="shrink-0 border-amber-500/30 bg-amber-500/10 text-amber-400">
                            <Coins className="mr-1 h-3 w-3" />{skill.coinValue}
                          </Badge>
                        </div>
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          <Badge variant="secondary" className="text-xs">{skill.category.replace(/_/g, " ")}</Badge>
                          {skill.level && (
                            <Badge variant="outline" className={`text-xs ${
                              skill.level === "EXPERT" ? "border-purple-500/20 bg-purple-500/10 text-purple-400" :
                              skill.level === "INTERMEDIATE" ? "border-amber-500/20 bg-amber-500/10 text-amber-400" :
                              "border-green-500/20 bg-green-500/10 text-green-400"
                            }`}>{skill.level}</Badge>
                          )}
                        </div>
                        {skill.description && <p className="mt-3 line-clamp-2 text-sm text-gray-400">{skill.description}</p>}
                      </CardContent>
                    </Card>
                  ))}
                </div>
            ...
            <TabsContent value="needed" className="mt-6">
              {needed.length === 0 ? (
                <Card className="border-white/10 bg-white/[0.03] py-16 text-center">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {isOwnProfile ? "Add skills you're looking for help with." : "This user isn't looking for any skills right now."}
                  </p>
                </Card>
              ) : (
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {needed.map((skill) => (
                    <Card key={skill.id} className="group border-white/10 bg-white/[0.03] transition-all hover:border-purple-500/30 hover:bg-white/[0.06]">
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between gap-3">
                          <h3 className="font-semibold text-white">{skill.title}</h3>
                          <Badge variant="outline" className="shrink-0 border-amber-500/30 bg-amber-500/10 text-amber-400">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          <Badge variant="secondary" className="text-xs">{skill.category.replace(/_/g, " ")}</Badge>
                          {skill.level && <Badge className={`text-xs ${
                            skill.level === "EXPERT" ? "bg-purple-500/10 text-purple-400" :
                            skill.level === "INTERMEDIATE" ? "bg-amber-500/10 text-amber-400" :
        ...
        <section className="mt-12">
          <h2 className="mb-4 text-xl font-semibold flex items-center gap-2">
            ...
            {avgRating && <span className="text-sm text-gray-400 font-normal">· {avgRating.toFixed(1)} avg</span>}
          </h2>
                  ...
                  <CardContent className="p-5">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-8 w-8">
                        <AvatarImage src={r.reviewer.image ?? ""} />
                      ...
                      <div>
                        ...
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star key={s} className={`h-3 w-3 ${s <= r.rating ? "fill-yellow-400 text-yellow-400" : "text-gray-600"}`} />
                          ))}
                        </div>
                      </div>
                    </div>
                    {r.comment && <p className="mt-3 text-sm text-gray-400">{r.comment}</p>}
        ...
        {session && !isOwnProfile && (
          <div className="mt-12 flex justify-center">
            <ExchangeModal
              ...
              otherUserNeeded={needed.map((s) => ({ id: s.id, title: s.title, coinValue: s.coinValue }))}
              currentUserOffered={currentUserSkills?.skills ?? []}
            />
          </div>
        )}
        {isOwnProfile && session && (
          <div className="mt-12 flex justify-center">
            <Link href="/marketplace"
              className="inline-flex items-center justify-center rounded-md border border-white/10 bg-transparent px-8 py-3 text-sm font-medium text-gray-300 transition-colors hover:bg-white/10 hover:text-white">
              <Handshake className="mr-2 h-5 w-5" />Browse Marketplace
            </Link>
          </div>
        )}
      </div>
